...
const studentRegister = async (req, res) => {
    try {
        ...
        // Course validation bypassed because courses are dynamically assigned
        ...
        const courses = await Subject.find({ sclassName: mappedDepartment._id, school: adminID });
        const courseIds = courses.map(c => c._id);

        if (courseIds.length === 0) {
            return res.status(400).json({ message: 'No courses available for selected department. Please create courses first.' });
        }
    ...
    try {
        ...
        if (student) {
            ...
            if (validated) {
                ...
                student = await student.populate("courseIds", "subName subCode")
...
const getStudentDetail = async (req, res) => {
    try {
        let student = await Student.findById(req.params.id)
            ...
            .populate("courseIds", "subName subCode")
...
const createStudentByFaculty = async (req, res) => {
    try {
        ...
        // Course assigned automatically
            // Courses assigned automatically
        ...
        const courses = await Subject.find({ sclassName: mappedDepartment._id, school: faculty.school });
        const courseIds = courses.map(c => c._id);

        if (courseIds.length === 0) {
            return res.status(400).json({ message: 'No courses available for selected department.' });
        }
...
const { V2Student } = require('../models/student.model');
...
const createStudent = asyncHandler(async (req, res) => {
  ...
  if (enrolledCourseIds.length) {
    ...
    if (hasCrossDepartmentCourse) throw new ApiError(400, "V2Student can only enroll in courses from the same department");
  }
  ...
  const created = await V2Student.create({
    user: user._id,
  ...
  const item = await V2Student.findById(created._id)
...
const listStudents = asyncHandler(async (_req, res) => {
  const items = await V2Student.find()
    .populate("user", "name email role isActive")
    .populate("department", "name code")
    .populate("enrolledCourses", "name code credits")
    .sort({ createdAt: -1 });
...
const getStudentV2 = asyncHandler(async (req, res) => {
  const item = await V2Student.findById(req.params.id)
    .populate("user", "name email role isActive")
    .populate("department", "name code")
    .populate("enrolledCourses", "name code credits");
  if (!item) throw new ApiError(404, "V2Student not found");
  res.status(200).json({ item });
});

const updateStudentV2 = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { name, email, isActive, registerNumber, departmentId, enrolledCourseIds } = req.body;

  const student = await V2Student.findById(id);
  if (!student) throw new ApiError(404, "V2Student not found");
  ...
  if (Object.keys(userUpdate).length) {
    ...
    if (!user) throw new ApiError(500, "V2Student user missing");
    Object.assign(user, userUpdate);
    await user.save();
  }

  const item = await V2Student.findById(id)
    .populate("user", "name email role isActive")
    .populate("department", "name code")
    .populate("enrolledCourses", "name code credits");
  res.status(200).json({ item });
...
const deleteStudentV2 = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const student = await V2Student.findById(id);
  if (!student) throw new ApiError(404, "V2Student not found");
  await V2Student.findByIdAndDelete(id);
...
module.exports = {
  ...
  createStudent,
  listStudents,
  getStudent: getStudentV2,
  updateStudentV2: updateStudentV2,
  deleteStudentV2: deleteStudentV2
};
